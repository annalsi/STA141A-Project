---
title: "STA141A Final Project: Predicting the Feedback Type of a Mice Performing a Visual Discrimination Task"
author: "Anna Si"
date: "2024-03-05"
output: 
  html_document:
...
```
```{r xg-boost-model, echo=FALSE, message=FALSE}
set.seed(119)
...
```{r session18test, echo=FALSE, message = FALSE}
# split
set.seed(119) # for reproducibility
...
# Section 6: Discussion
  ...
  Based off the test data, the area under the curve from test1 for Session 1 was 0.69 while my accuracy was 76%. Thus this is a poor model to use to predict whether a trial from session 1 will result in a success. In addition, the auc for test2 for Session 18 was 0.51 while accuracy was 69% which also suggests that the model is not great at predicting whether a trial from session 18 will result in a success since it would be similar to guessing. I hypothesize that the reason why my model did not function as well as it should have was because it predicted a large amount of false positives and almost no true negative values. However, while it did not reach as good as my test results which was 78% accuracy, I do think this model does work to predict feedback, just not that accurately, perhaps because we need more predictors that influence feedback type. Thus, my model is not as accurate as I would have liked and it would be best to try a new model that would better predict the feedback type given the data or perhaps to create two separate models, using the respective data from session 1 and 18 to train two models.
...
ggplot(average_spike_success, aes(x = trail_id, y = mean_spike)) + geom_line()+ geom_smooth(method = "loess")+ facet_wrap(~session_id) + labs(title = "Average Spike Rate for Successful Trials")

# average spike for failure trials
average_spike_fail <- full_tibble %>% filter(success == 0) %>% group_by( session_id,trail_id) %>% summarise(mean_spike = sum(region_sum_spike)/sum(region_count))

ggplot(average_spike_fail, aes(x = trail_id, y = mean_spike)) + geom_line() +
  geom_smooth(method = "loess") + facet_wrap(~session_id) + labs(title = "Average Spike Rate for Failure Trials")
```
...
# References
- Github link: https://github.com/annalsi/STA141A-Project.git
...
- Understanding PCA:
  - https://hbctraining.github.io/DGE_workshop/lessons/principal_component_analysis.html#:~:text=These%20axes%20that%20represent%20the,which%20we%20could%20have%20variation.
  - https://www.youtube.com/watch?v=3QwZ2GgHSLE
I heavily referenced the TA's Project Demo 1 and Demo 2 along with one of the Consulation Sessions.
- Chat GPT Convo
User
how to plot roc curve for an xgboost model and logistic regression model in r

ChatGPT
In R, you can also plot ROC curves for models like XGBoost and logistic regression. You can use libraries like pROC for computing ROC curve metrics and ggplot2 for plotting. Here's a step-by-step guide to achieve this:

Install Required Packages: First, if you haven't already, you need to install the necessary packages pROC and ggplot2.
R
Copy code
install.packages("pROC")
install.packages("ggplot2")
Load Required Libraries: Load the required libraries into your R environment.
R
Copy code
library(pROC)
library(ggplot2)
Train the Models: Train both the XGBoost and logistic regression models using your dataset.

Predict Probabilities: Generate predicted probabilities for both models on a holdout dataset.

Compute ROC Curve Metrics: Use the roc function from the pROC package to compute the ROC curve metrics for both models.

Plot ROC Curve: Finally, use ggplot2 to plot the ROC curves for both models.

Here's a code example:

R
Copy code
 Assume you have already trained models and have test data
 Here's an example with random data for illustration purposes
...
log_reg_model <- glm(y_train ~ ., data=data.frame(cbind(y_train, X_train)), family=binomial)

 Train XGBoost model (assuming you have installed the xgboost package)
 install.packages("xgboost")
library(xgboost)
xgb_model <- xgboost(data = X_train, label = y_train, nrounds = 10, verbose = 0)

 Predict probabilities for logistic regression
y_pred_proba_lr <- predict(log_reg_model, newdata=data.frame(X_test), type="response")

 Predict probabilities for XGBoost
y_pred_proba_xgb <- predict(xgb_model, newdata = as.matrix(X_test))

 Compute ROC curve metrics
roc_lr <- roc(y_test, y_pred_proba_lr)
roc_xgb <- roc(y_test, y_pred_proba_xgb)

 Plot ROC curve
plot_roc <- ggroc(list(Logistic_Regression = roc_lr, XGBoost = roc_xgb)) +
